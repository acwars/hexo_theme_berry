...
hexo.extend.helper.register('related_posts', function (currentPost, allPosts) {
  ...
  const limitNum = config.related_post.limit || 3
  ...
  if (relatedPosts.length > 0) {
    ...
    result +=
      '<div class="relatedPosts_headline"><span>' +
      ' ' +
      '</span></div>'
    ...
    for (let i = 0; i < Math.min(relatedPosts.length, limitNum); i++) {
      ...
      if (dateType === 'created') {
        result +=
          '<div class="relatedPosts_main is-center"><div class="relatedPosts_date"><i class="far fa-calendar-check fa-fw"></i>' +
          ' ' +
          this.date(relatedPosts[i].created, hexoConfig.date_format) +
          '</div>'
      } else {
        result +=
          '<div class="relatedPosts_main is-center"><div class="relatedPosts_date"><i class="far fa-calendar-check"></i>' +
          ' ' +
          this.date(relatedPosts[i].modified, hexoConfig.date_format) +
          '</div>'
      }
      result +=
